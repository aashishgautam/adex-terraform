...
module "elb" {
  ...
  log_group_name        = "/aws/alb/staging-alb-access-logs"
...
module "ecs-task-service" {
  ...
  service_name                      = "nodejs-todo-staging"
  desired_count                     = 2
  deployment_max                    = 200
  deployment_min                    = 100
  container_name                    = "nodejs-todo-staging"
  container_port                    = 3000
  ...
  name                              = "task-log"
  project_name                      = "assignment"
  task_log_group                    = "/aws/tasks/staging"
}
...
module "iam" {
  ...
  project_name               = "assignment"
  creator_name               = "aashish"
  task_execution_role_name   = "staging-task-execution-role-ecs"
  instance_profile_name      = "staging-instance-profile"
  instance_profile_role_name = "staging-instance-profile-role"
}
